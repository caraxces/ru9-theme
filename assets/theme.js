...
(function () {
  ...
  theme.Variants = (function () {
    function Variants(options) {
      ...
      this.originalSelectorIdSticky = options.originalSelectorIdSticky;
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _updateMasterSelect: function (variant) {
        ...
        let masterSelectSticky = this.container.querySelector(
          this.originalSelectorIdSticky
        );
        ...
        // sticky add to cart
        if (!masterSelectSticky) return;

        masterSelectSticky.value = variant.id;

        // Force a change event so Shop Pay installments works after a variant is changed
        masterSelectSticky.dispatchEvent(
  ...
  theme.CartForm = (function () {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      initRemoveCart: function () {
        this.form.querySelectorAll(".cart__remove a").forEach((el) => {
  ...
  theme.collapsibles = (function () {
    var selectors = {
      ...
      triggerClose: ".collapsible-content__inner-close",
    ...
    function init(scope) {
      ...
      el.querySelectorAll(selectors.trigger).forEach((trigger) => {
        ...
        // If trigger has class 'is-open', set height immediately
        if (state) {
          const container = trigger.parentElement.querySelector(
            selectors.module
          )
          ...
          if (!contentInner) {
            console.warn('Không tìm thấy phần tử moduleInner trong container');
            return;
          }
          container.style.height = contentInner.offsetHeight + "px";
          container.classList.add(classes.open);
        }

        trigger.off("click" + namespace);

        if (!trigger.closest(".filter-form")) {
          trigger.on("click" + namespace, toggle);
        }

        if (trigger.closest(".filter-form") && window.innerWidth >= 960) {
          trigger.on("click" + namespace, toggle);
        } else {
          ...
          el.querySelectorAll(selectors.triggerClose).forEach((trigger) => {
            trigger.addEventListener("click", (e) => {
              e.preventDefault();
              ...
              if (trigger.closest("#FilterDrawer")) {
                trigger
                  .closest("#FilterDrawer")
                  .querySelectorAll(selectors.module)
                  .forEach((item) => {
      ...
      open: function (evt, returnFocusEl) {
        ...
        theme.utils.prepareTransition(
          ...
          function () {
            ...
            this.drawer
              .querySelectorAll(".collapsible-content--sidebar")
              .forEach((item) => {
                if (item) {
  ...
  theme.AjaxProduct = (function () {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      addItemFromForm: function (evt, callback) {
        ...
        if (window.cartBundle && window.cartBundle.length) {
          window.cartBox = window.cartBox || [];
          window.cartBundle.map((item) => {
            window.cartBox.push({
        ...
        if (window.cartBox && window.cartBox.length > 0) {
          ...
          if (window.cartBundle && window.cartBundle.length > 0) {
            formData.delete("id");
            formData.delete("quantity");
          }
          config.body = formData;
          fetch(theme.routes.cartAdd, config)
            .then((response) => response.json())
            .then(
              function (data) {
                if (data.status === 422) {
    ...
    slideshow.prototype = Object.assign({}, slideshow.prototype, {
      ...
      childNavGoTo: function (index) {
        ...
        if (this.args.childVertical) {
          ...
          if (window.matchMedia("(max-width: 959px)").matches === true) {
            if (
              this.args.childNavScroller.closest(".product__thumbs").dataset
                .positionTablet === "beside"
            ) {
              var elTop = el.offsetTop;
              this.args.childNavScroller.scrollTop = elTop - 100;
            }
...
==============================================================================*/
  ...
  class ThemeTab extends HTMLElement {
    constructor() {
      ...
      this.panel = this.querySelectorAll(".tab__item-panel");

      this.panel.forEach((panel) => {
        panel.addEventListener("click", () => {
          const id_tab = panel.getAttribute("id");
          this.open(panel, id_tab);
        });
      });
    }
    ...
    open(button, tab) {
      ...
      if (_target.classList.contains("is-active")) return;

      const _active = this.querySelector(`li.is-active`);
      const _activeTab = this.querySelectorAll(`.tab__content-item.is-active`);
      const _targetTab = this.querySelectorAll(`[data-tab-id="${tab}"]`);

      _active.classList.remove("is-active");
      _target.classList.add("is-active");

      _activeTab.forEach((el) => el.classList.remove("is-active"));
      _targetTab.forEach((el) => el.classList.add("is-active"));
  ...
  theme.headerNavMobile = (function () {
    var selectors = {
      hasDropdownClass: ".mobile-nav__has-sublist button",
      hasSubDropdownClass: ".site-nav__deep-dropdown-trigger",
      closeButton: ".js-megamenu-close",
      subDropdown: ".mobile-nav__sublist",
    };
    var classes = {
      dropdownActive: "is-opened",
    };
    ...
    function showSubmenu(e) {
      e.preventDefault();
      document
        .querySelectorAll(selectors.subDropdown)
        .forEach((el) => el.classList.remove(classes.dropdownActive));
      let id = e.currentTarget.getAttribute("aria-controls");
      // document.querySelector('.drawer__fixed-header').classList.add(classes.dropdownActive)
      document
        .querySelector(`.mobile-nav #${id}`)
        .classList.add(classes.dropdownActive);
    }
    function closeSubmenu(e) {
      e.preventDefault();
      document
        .querySelector(".drawer__fixed-header")
        .classList.remove(classes.dropdownActive);

      document.querySelectorAll(selectors.subDropdown).forEach((el) => {
    ...
    function init() {
      document
        .querySelectorAll(selectors.hasDropdownClass)
        .forEach((el) => el.addEventListener("click", showSubmenu));
      document
        .querySelectorAll(selectors.closeButton)
        .forEach((el) => el.addEventListener("click", closeSubmenu));
    }
    ...
    SlideshowSection.prototype = Object.assign({}, SlideshowSection.prototype, {
      init: function () {
        ...
        if (slides.length > 1) {
          ...
          var carousel = document.querySelector(".hero");
          var slides = document.getElementsByClassName("hero__image-wrapper");
      ...
      onDeselect: function () {
        if (flkty && typeof flkty.play === "function") {
          flkty.play();
        }
      },
      ...
      onBlockSelect: function (evt) {
        ...
        if (flkty && typeof flkty.pause === "function") {
          flkty.goToSlide(index);
          flkty.pause();
        } else {
  ...
  theme.MasonrySection = (function () {
    function MasonrySection(container) {
      this.container = container;
      var sectionId = container.getAttribute("data-section-id");
      this.slideshow = container.querySelector("#Masonry-" + sectionId);
      this.namespace = "." + sectionId;
    ...
    MasonrySection.prototype = Object.assign({}, MasonrySection.prototype, {
      init: function () {
        var slides = this.slideshow.querySelectorAll(".banner");
  ...
  theme.CollectionTabSection = (function () {
    function CollectionTabSection(container) {
      this.container = container;
      var sectionId = container.getAttribute("data-section-id");
      this.slideshow = container.querySelectorAll(".tab__content-slider");
      this.namespace = "." + sectionId;
    ...
    CollectionTabSection.prototype = Object.assign(
      {},
      CollectionTabSection.prototype,
      {
        init: function () {
          this.slideshow.forEach((slideshow) => {
            var slides = slideshow.querySelectorAll(".gird__item");
            ...
            if (window.matchMedia("(max-width: 959px)").matches == true) {
              this.flickity = new theme.Slideshow(slideshow, sliderArgsMobile);
            } else {
              this.flickity = new theme.Slideshow(slideshow, sliderArgs);
            }
          });
        },
  ...
  theme.BannerWithCollectionSection = (function () {
    function BannerWithCollectionSection(container) {
      this.container = container;
      var sectionId = container.getAttribute("data-section-id");
      this.slideshow = container.querySelector(
        "#Banner-Collection-" + sectionId
      );
      this.namespace = "." + sectionId;
    ...
    BannerWithCollectionSection.prototype = Object.assign(
      {},
      BannerWithCollectionSection.prototype,
      {
        init: function () {
          var slides = this.slideshow.querySelectorAll(".collection-item");
    ...
    return BannerWithCollectionSection;
  })();

  theme.CertificationSection = (function () {
    function CertificationSection(container) {
      this.container = container;
      var sectionId = container.getAttribute("data-section-id");
      this.slideshow = container.querySelector("#Certification-" + sectionId);
      this.namespace = "." + sectionId;
    ...
    CertificationSection.prototype = Object.assign(
      {},
      CertificationSection.prototype,
      {
        init: function () {
          var slides = this.slideshow.querySelectorAll(".certification-item");
  ...
  theme.BlogPostSection = (function () {
    function BlogPostSection(container) {
      this.container = container;
      var sectionId = container.getAttribute("data-section-id");
      this.slideshow = container.querySelector("#Blogpost-Slider-" + sectionId);
      this.namespace = "." + sectionId;
    ...
    BlogPostSection.prototype = Object.assign({}, BlogPostSection.prototype, {
      init: function () {
        var slides = this.slideshow.querySelectorAll(".grid__item");
  ...
  theme.Collection = (function () {
    ...
    var selectors = {
      ...
      priceRange: ".price-range",
      facetClear: ".collection-facets-clear",
    };
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      initSort: function () {
        ...
        this.sortSelectMobile = document.querySelector(
          selectors.sortSelectMobile
        );
        ...
        if (window.innerWidth < 768) {
          if (this.sortSelectMobile) {
            ...
            this.sortSelectMobile.on(
              "change" + this.namespace,
              this.onSortChangeMobile.bind(this)
            );
          }
        }
      },

      getSortValueMobile: function () {
        return this.sortSelectMobile.value || this.defaultSortMobile;
      },
      ...
      getDefaultSortValueMobile: function () {
        return this.sortSelectMobile.getAttribute("data-default-sortby-mobile");
      },
  ...
  theme.Product = (function () {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        formSticky: ".product-single__sticky-form",
        size_select: 'select[data-type="Kích thước"]',
        ...
        installmentPrice: "[data-installment-price]",
        ...
        variantSelectedLabel: "[data-variant-selected-label]",

        addToCart: "[data-add-to-cart]",
        addToCartText: "[data-add-to-cart-text]",
        addToCartSticky: "[data-add-to-cart-sticky]",
        addToCartTextSticky: "[data-add-to-cart-text-sticky]",

        originalSelectorId: "[data-product-select]",
        originalSelectorIdSticky: "[data-product-select-sticky]",
        singleOptionSelector: "[data-variant-input]",
        variantColorSwatch:
          ".product-block:not(.product-block-bundle) .variant__input--color-swatch",
        variantPill: ".variant__input--pill",
        dynamicVariantsEnabled: "[data-dynamic-variants-enabled]",
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      init: function () {
        ...
        const product_combo = this.container.querySelector(".product-combo");
        if (product_combo) {
          const price_cb_result = this.container.querySelector(
            ".money-combo-result"
          );
          const price_cb_percent = this.container.querySelector(
            ".money-combo-percent"
          );
          const price_cb_compare = this.container.querySelector(
            ".money-combo-compare"
          );
          const percent = price_cb_result.getAttribute(
            "data-product-combo-discount"
          );

          if (window.cartBox && window.cartBox.length > 0) {
            const totalPriceCombo = window.cartBox.reduce(
              (total, item) => total + item.price,
              0
            );
            const resultPrice =
              totalPriceCombo +
              Number(this.cache.price.getAttribute("data-product-price-base"));
            const savingsCombo = Math.round(resultPrice / Number(percent));

            // this.cache.price.setAttribute('data-product-price-base', this.cache.price.getAttribute('data-product-price-base'))

            price_cb_percent.innerHTML = theme.Currency.formatMoney(
              savingsCombo,
              theme.settings.moneyFormat
            );
            price_cb_compare.innerHTML = theme.Currency.formatMoney(
              resultPrice,
              theme.settings.moneyFormat
            );
            price_cb_result.innerHTML = theme.Currency.formatMoney(
              resultPrice - savingsCombo,
              theme.settings.moneyFormat
            );
          }
        }
      ...
      cacheElements: function () {
        this.cache = {
          ...
          formSticky: this.container.querySelector(this.selectors.formSticky),
      ...
      initVariants: function () {
        ...
        var options = {
          ...
          originalSelectorIdSticky: this.selectors.originalSelectorIdSticky,
        ...
        var variantsPill = this.container.querySelectorAll(
          this.selectors.variantPill
        );

        if (variantsPill.length) {
          variantsPill.forEach((pill) => {
            pill.addEventListener(
              "change",
              function (evt) {
                var value = pill.dataset.valueName;
                var index = pill.dataset.valueIndex;
                this.updateSelectedName(pill, value, index);
              }.bind(this)
            );
          });
        }
      ...
      initAjaxProductForm: function () {
        ...
        new theme.AjaxProduct(this.cache.formSticky);
      },
      ...
      updateSelectedName: function (selector, name, index) {
        // Updates on radio button change, not variant.js
        selector
          .closest(".variant-wrapper")
          .querySelector(
            this.selectors.variantSelectedLabel + `[data-index="${index}"`
          ).textContent = name;
      },

      updateSticky: function (evt) {
        ...
        if (stickyAddToCart) {
          const labelOptions =
            stickyAddToCart.querySelectorAll(".label-select");
          console.log(variant);
          ...
          if (labelOptions[0]) {
            labelOptions[0].querySelector("[data-header-option]").textContent =
              option1;
          }
          if (labelOptions[1]) {
            labelOptions[1].querySelector("[data-header-option]").textContent =
              option2;
          }
          if (labelOptions[2]) {
            labelOptions[2].querySelector("[data-header-option]").textContent =
              option3;
          }
        }
      },
      ...
      updateCartButton: function (evt) {
        ...
        var cartBtnSticky = this.container.querySelector(
          this.selectors.addToCartSticky
        );
        var cartBtnTextSticky = this.container.querySelector(
          this.selectors.addToCartTextSticky
        );
        const size = this.container.querySelector(this.selectors.size_select);

        if (!cartBtn) return;

        if (evt.detail.disabled) {
          cartBtn.classList.add(classes.disabled);
          cartBtn.disabled = true;

          if (cartBtnSticky) {
            cartBtnSticky.classList.add(classes.disabled);
            cartBtnSticky.disabled = true;
          }
          return;
        }
        if (evt.detail.soldOut) {
          cartBtn.classList.add(classes.disabled);
          cartBtn.disabled = true;
          ...
          if (cartBtnSticky) {
            cartBtnSticky.classList.add(classes.disabled);
            cartBtnSticky.disabled = true;
            cartBtnTextSticky.textContent = theme.strings.soldOut;
          }
          return;
        }

        if (size) {
          const currentSize = size.value.split("-")[0];
          document.querySelectorAll("product-bundle").forEach((bundle) => {
            if (bundle && bundle.updateSize) {
              bundle.updateSize(currentSize);
            }
          });
        }
        ...
        if (variant) {
          if (variant.available) {
            ...
            if (cartBtnSticky) {
              cartBtnSticky.classList.remove(classes.disabled);
              cartBtnSticky.disabled = false;
              var defaultStickyText = cartBtnTextSticky.dataset.defaultText;
              cartBtnTextSticky.textContent = defaultStickyText;
            }
          } else {
          ...
          if (cartBtnSticky) {
            cartBtnSticky.classList.add(classes.disabled);
            cartBtnSticky.disabled = true;
            cartBtnTextSticky.textContent = theme.strings.unavailable;
          }
        }
      },
      ...
      updatePrice: function (evt) {
        var variant = evt.detail.variant;
        const installmentWrapper = this.container.querySelector(
          ".product-block--price"
        );
        const minInstallmentPrice = 300000;
        ...
        if (variant) {
          ...
          if (this.cacheDuplicate.installmentPrice)
            this.cacheDuplicate.installmentPrice.innerHTML =
              theme.Currency.formatMoney(
                Math.round(variant.price / 12),
                "{{amount}}"
              );
          ...
          if (product_combo) {
            ...
            if (window.cartBox && window.cartBox.length > 0) {
              ...
              const resultPrice = totalPriceCombo + variant.price;
              const savingsCombo = Math.round(resultPrice / Number(percent));

              this.cache.price.setAttribute(
                "data-product-price-base",
                variant.price
              );
      ...
      updateVariantImage: function (evt) {
        ...
        // Display image that variant image's stacked
        if (this.cache.mainSlider.classList.contains("prevent-slider")) {
          if (window.matchMedia("(min-width: 960px)").matches === true) {
            console.log(imageIndex);
            // this.cache.mainSlider.querySelectorAll('.product-main-slide')[imageIndex].style.display = 'block'
            this.cache.mainSlider
              .querySelectorAll(".product-main-slide")
              .forEach((slide) => {
                slide.style.display = "none";
                if (slide.getAttribute("data-index") == imageIndex) {
  ...
  DOMready(function () {
    ...
    // new section
    theme.sections.register("masonry-section", theme.MasonrySection);
    theme.sections.register("f-col-tab-section", theme.CollectionTabSection);
    theme.sections.register(
      "banner-w-coll-section",
      theme.BannerWithCollectionSection
    );
    theme.sections.register(
      "certification-section",
      theme.CertificationSection
    );
    theme.sections.register("blogposts-section", theme.BlogPostSection);
    // end new section
